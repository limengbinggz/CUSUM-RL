		...
		for s in ${!np_random_seeds[*]}; do
			...
			for k in ${!kappas[@]}; do
				...
				for r in ${!RBFSampler_random_states[@]}; do
					...
					python 01_sim_ihs_run.py ${seed} ${kappa} ${gamma} ${N} ${RBFSampler_random_state}
				done 
			done
		done
	done
	...
	for g in ${!gammas[*]}; do
		...
		for s in ${!np_random_seeds[*]}; do
			...
			for type_est in "${type_ests[@]}"; do
				...
				python 04_sim_ihs_changept_optvalue_run.py ${seed} ${gamma} ${N} ${type_est}
			done
		done
	done
done

echo "summarise the values of different policies"
